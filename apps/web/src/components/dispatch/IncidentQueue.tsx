...
const IncidentQueue: React.FC<IncidentQueueProps> = ({
  ...
  // Notify parent component when incidents change
  useEffect(() => {
    if (onIncidentsUpdate) {
      onIncidentsUpdate(incidents);
    }
  }, [incidents, onIncidentsUpdate]);
  ...
  // Auto-scroll to selected incident
  useEffect(() => {
    if (selectedIncidentId && incidentRefs.current[selectedIncidentId]) {
      const element = incidentRefs.current[selectedIncidentId];
      if (element) {
      ...
      <div className="flex-shrink-0 px-3 py-2 border-b bg-gray-50">
        ...
        <div className="flex gap-1 text-xs">
          <select
            ...
            className="px-2 py-1 border border-gray-300 rounded text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 pr-6"
          >
          ...
          <div className="space-y-1 p-2">
            {filteredAndSortedIncidents.map((incident) => (
              <div
                key={incident._id}
                ref={(el) => {
                  incidentRefs.current[incident._id] = el;
                }}
                onClick={() => handleIncidentClick(incident)}
                className={`
                  p-2 border-2 rounded cursor-pointer transition-all duration-200 hover:shadow-sm
