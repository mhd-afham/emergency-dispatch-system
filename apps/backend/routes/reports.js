  ...
  try {
    ...
    if (resolvedIncidents.length > 0) {
      ...
      if (validIncidents.length > 0) {
        ...
        averageResponseTime = (totalResponseTime / validIncidents.length).toFixed(1) + ' minutes';
      }
    }
    ...
    // Vehicle status
    const vehicleStats = {
      ready: await Vehicle.countDocuments({ isActive: true, 'status.operational': 'active' }),
      maintenance: await Vehicle.countDocuments({ isActive: true, 'status.operational': 'maintenance' }),
      outOfService: await Vehicle.countDocuments({ isActive: true, 'status.operational': 'out_of_service' }),
      total: await Vehicle.countDocuments({ isActive: true })
    };

    // Crew status
    const crewStats = {
      available: await Crew.countDocuments({ isActive: true, 'currentStatus.availability': 'available' }),
      onDuty: await Crew.countDocuments({ isActive: true, 'currentStatus.availability': 'on_duty' }),
      total: await Crew.countDocuments({ isActive: true })
    };

    // Top locations
    const locationHotspots = await Incident.aggregate([
      { $match: { createdAt: { $gte: startDate, $lt: endDate }, 'location.city': { $exists: true, $ne: null, $ne: '' } } },
    ...
    const doc = new PDFDocument({ 
      ...
      bufferPages: true,
      info: {
        Title: `Analytics ${range.charAt(0).toUpperCase() + range.slice(1)} Report`,
    ...
    const filename = `Respondr_Analytics_${rangeLabel}_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    doc.fillColor('#ffffff')
       .rect(logoX - 2, logoY - logoSize / 3, 4, logoSize * 2 / 3).fill()
       .rect(logoX - logoSize / 3, logoY - 2, logoSize * 2 / 3, 4).fill();
    ...
    doc.fontSize(7)
       .fillColor('#6b7280')
       .font('Helvetica')
       .text(`Period: ${startDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} - ${endDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`, 85, 48, { 
         width: 400,
         align: 'left'
       });
    ...
    summaryData.forEach((item, index) => {
      ...
      doc.fontSize(8)
         .fillColor('#111827')
         .font('Helvetica-Bold')
         .text(String(item.value), 350, tableY + 4, { width: 180, align: 'right' });
    ...
    const statusMap = { pending: 0, assigned: 0, en_route: 0, on_scene: 0, resolved: 0 };
    ...
    statusItems.forEach((item, index) => {
      ...
      doc.fontSize(8)
         .fillColor('#111827')
         .font('Helvetica-Bold')
         .text(String(item.value), 450, statusY + 3, { width: 80, align: 'right' });
    ...
    const typeMap = { medical: 0, fire: 0, rescue: 0, hazmat: 0, traffic: 0, other: 0 };
    ...
    typeItems.forEach((item, index) => {
      ...
      doc.fontSize(8)
         .fillColor('#111827')
         .font('Helvetica-Bold')
         .text(String(item.value), 450, typeY + 3, { width: 80, align: 'right' });
    ...
    doc.font('Helvetica-Bold').text(String(vehicleStats.ready), 220, resourceY + 22, { width: 60, align: 'right' });
    
    doc.font('Helvetica').text('Under Maintenance', 60, resourceY + 34, { width: 150 });
    doc.font('Helvetica-Bold').text(String(vehicleStats.maintenance), 220, resourceY + 34, { width: 60, align: 'right' });
    
    doc.font('Helvetica').text('Out of Service', 60, resourceY + 46, { width: 150 });
    doc.font('Helvetica-Bold').text(String(vehicleStats.outOfService), 220, resourceY + 46, { width: 60, align: 'right' });
    
    // Divider line
    doc.moveTo(60, resourceY + 58).lineTo(280, resourceY + 58).strokeColor('#cbd5e1').lineWidth(0.5).stroke();
    
    doc.fontSize(7).fillColor('#111827').font('Helvetica-Bold')
       .text('Total Units', 60, resourceY + 61, { width: 150 });
    doc.text(String(vehicleStats.total), 220, resourceY + 61, { width: 60, align: 'right' });
    ...
    doc.font('Helvetica-Bold').text(String(crewStats.available), 475, resourceY + 22, { width: 60, align: 'right' });
    
    doc.font('Helvetica').text('On Active Duty', 315, resourceY + 34, { width: 150 });
    doc.font('Helvetica-Bold').text(String(crewStats.onDuty), 475, resourceY + 34, { width: 60, align: 'right' });
    
    // Divider line
    doc.moveTo(315, resourceY + 50).lineTo(535, resourceY + 50).strokeColor('#cbd5e1').lineWidth(0.5).stroke();
    
    doc.fontSize(7).fillColor('#111827').font('Helvetica-Bold')
       .text('Total Personnel', 315, resourceY + 54, { width: 150 });
    doc.text(String(crewStats.total), 475, resourceY + 54, { width: 60, align: 'right' });
    ...
    if (locationHotspots.length > 0) {
      ...
      locationHotspots.forEach((loc, index) => {
        ...
        doc.fontSize(8)
           .fillColor('#111827')
           .font('Helvetica-Bold')
           .text(`${loc.count} incident${loc.count !== 1 ? 's' : ''}`, 420, locY + 3, { width: 110, align: 'right' });
    ...
    const addFooter = (pageNum) => {
      ...
      doc.fontSize(6)
         .fillColor('#9ca3af')
         .font('Helvetica')
         .text('Emergency: 1-1-9 | support@respondr.lk | www.respondr.lk', 50, footerTop + 18, { 
           width: 495, 
           align: 'center',
           lineBreak: false
         });
      ...
      doc.fontSize(6)
         .fillColor('#9ca3af')
         .text(`Generated: ${generatedDate} | Page ${pageNum}`, 50, footerTop + 28, { 
           width: 495, 
           align: 'center',
           lineBreak: false
         });
